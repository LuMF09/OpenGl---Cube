...
void MainView::timerUpdate() {
 ...
 void MainView::updateCube(glm::vec3 p1, glm::vec3 p2)
 {
     //Write two points of the line
     scene->writeLine(p1,p2);
     //Display the line
     scene->CreateVBOLine();
     //Focus the camera on the center of the cube
     scene->updateEyeCameraOnCube(scene->getCenter());
 }

 void MainView::rotateMatrix(setOfValues param)
 {
    //rotation of the matrix
    scene->rotateModel(param);
 }

 void MainView::updateCamera(setOfView param)
 {
     //Set the camera with the inputs from the user
     scene->updateEyeCameraUser(param);
 }

 void MainView::restoreCamera()
 {
     //restore default parameters of the camera
     scene->restoreCamDefault();
 }
